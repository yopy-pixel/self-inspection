...
1. **いま使っている端末**のアプリ → 設定 → **Add a device** → 8文字のコードが出る
2. **新しい端末**に同じ APK を入れて起動 → 設定 → **Endpoint (https)** を入れる
3. **Pairing code** にコードを打ち込んで **Register**
4. `Device ID` と `Token` が自動で埋まり、同期が有効になって保存される

> コードは**端末用**と**ブラウザ用**で別です。取り違えると通りません（401）。
> 端末用コードは書き込み権限を配るため、意図的に分けてあります。
